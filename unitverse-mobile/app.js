...
// ── GLOBAL ERROR CAPTURE (holístico, evita que fallos silencien el resto) ─────
window.addEventListener('error', event => {
  console.error('[UnitVerse] Error global detectado:', event.message, 'en', event.filename, 'línea', event.lineno);
  const debug = document.getElementById('dgz-firma');
  if (debug) debug.textContent = 'ERROR JS detectado: ' + event.message;
});
window.addEventListener('unhandledrejection', event => {
  console.error('[UnitVerse] Promise no manejada:', event.reason);
  const debug = document.getElementById('dgz-firma');
  if (debug) debug.textContent = 'PROMISE REJECT: ' + event.reason;
});
...
function openPanel(id) {
  if (!id || typeof id !== 'string') { console.warn('[UnitVerse] openPanel id inválido:', id); return; }
  ...
  if (!next) {
    console.warn('[UnitVerse] panel no encontrado:', id);
    return;
  }
...
function openMenu() {
  ...
  const moreBtn = document.querySelector('.sb-more-btn');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const navOv = document.getElementById('mn-overlay');
  if (navOv) navOv.addEventListener('click', closeMenu);

  document.querySelectorAll('.mn-drawer-item').forEach(item => {
    item.addEventListener('click', () => {
      const panel = item.dataset.panel;
      if (panel && typeof openPanel === 'function') openPanel(panel);
      closeMenu();
    });
  });
});
